  ...
  async presentLocalNotification(date) {
    function sendNotification() {
      ...
      Notifications.scheduleNotificationAsync({
        ...
        trigger: null // makes notification trigger immediately
      });
    }
    let permissions = await Notifications.getPermissionsAsync();  // await is ES8 keyword that can only be used in async function, followed by a promise
    if (!permissions.granted) {
      permissions = await Notifications.requestPermissionsAsync();  // try one more time
    }
    if (permissions.granted) {
      sendNotification();
    }
